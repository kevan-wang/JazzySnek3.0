...
///		############ GAME SETTINGS ############
	...
	*/
	...
	function gameOver() {
		...
		var SScore = parseInt($("#SpideyScore").text())
		var combinedScore = JScore + SScore;
		var bool = combinedScore == 64;
		return bool
	}
	...
	*/	
	...
	/*	FUNCTION:  High-level function that converts all tiles from a coordinate, along a direction.
		...
		INPUT:  
			...
			player:  "N" for neutral, "J" for JazzySnek, "S" for Spidey.
		OUTPUT:  Score gained.  An integer.  Note that this does NOT include the tile that was placed!!!
	*/
	function takeVector(coords, direction, player) {
		var y = coords[0], x = coords[1];
		...
		while ( y > 0 && x > 0 && y < 9 && x < 9 ) {
			...
			score++;
			y += vectIncrement[0]	//	Incrementing position in given direction.
			x += vectIncrement[1]
			if (checkOwner([y, x]) == player) {	// If you hit a tile you own, that's it!
				break;
			}
		}
		return score;
	...
	/*	FUNCTION:  High-level function that evaluates the score that will result from this move,
			along with all legal attack vectors.  0 means illegal move.  Positive # means a legal move.
		INPUT:  
			coords:  [y,x] coordinates.  Y and X are integers from 1 to 8, with origin at top left corner.
			player:  "N" for neutral, "J" for JazzySnek, "S" for Spidey.
		OUTPUT:  [score, vectors].  
			score:  Score to be gained, all legal attack vectors in the form of 
			vectors:  A list of 1-2 char Strings representing cardinal directions, such as "NW" for Northwest
	*/
	...
	*/
	function checkVector(coords, direction, player) {
		var y = coords[0], x = coords[1]	// COPY the input coordinates. Don't want to modify the inputCoords object!
		var owner = checkOwner([y,x])
		if (owner != "N") {		//	If the space we're evaluating to place a tile is not free, return 0.
			...
			while ( y > 0 && x > 0 && y < 9 && x < 9 ) {
				// While the tile being investigated is within the dance floor (NOT on the border)
				if (checkOwner([y,x]) == "N") {	// Vector cannot end on an empty space.
					return 0;
				}
				else if (checkOwner([y,x]) == player) {	// If you hit a tile you own, that's it!
					break;
				}
	...
	*/
	function checkOwner(coords) {
		...
		if (tile == 1 || tile == 2) {
			...
			// For 8 and 9, you're on the border.  Check directly from HTML tags who owns this border tile.
			var id = "#" + y + x 
			return $(id).attr("name")
		}
	}
	...
	*/
	...
	/*	FUNCTION:  Used to get coords from an ID.  An ID is in the format "#yx" where Y and X are integers
			representing their respective coordinates.
		INPUT:  ID.
		OUTPUT:  [y,x] coordinates.  Y and X are integers from 1 to 8, with origin at top left corner.
	*/
